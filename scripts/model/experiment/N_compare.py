    ...
    try:
        ...
        for N in Ns:
            ...
            stats_true, stats_ana = run_EnKF(eta0, H_minus,
                                             ...
                                             obs_time_interval_sec=3.0, N=N,
                                             inflation_factor=1.05,
                                             save_dir="../../../result/N_compare"
                                             )
        ...
        # 最大波高のプロット
        ax1.plot(Ns, max_heights, marker='o', label='Estimated (Analysis)')
        ax1.axhline(y=true_max_h, color='r', linestyle='--', label='True Value')
        ax1.set_xlabel('Ensemble Size (N)')
        ax1.set_ylabel('Max Wave Height (m)')
        ax1.set_title('N vs Max Wave Height')
        ax1.legend()
        ax1.grid(True)

        # 到達時刻のプロット
        ax2.plot(Ns, arrival_times, marker='s', color='green', label='Estimated (Analysis)')
        ax2.axhline(y=true_arrival_t, color='r', linestyle='--', label='True Value')
        ax2.set_xlabel('Ensemble Size (N)')
        ax2.set_ylabel('Arrival Time (s)')
        ax2.set_title('N vs Tsunami Arrival Time')
        ax2.legend()
        ax2.grid(True)

        plt.tight_layout()
        plt.savefig("../../../result/N_compare/sensitivity_results.png")
